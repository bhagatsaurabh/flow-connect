...
export class Select extends UINode implements Serializable<SerializedSelect> {
  ...
  constructor(node: Node, public values: string[] = [], options: SelectOptions = DefaultSelectOptions(node)) {
    ...
    this.label = new Label(this.node, this.selected, {
  ...
  onClick(screenPosition: Vector, realPosition: Vector): void {
    ...
    let direction;
    if (realPosition.x < this.position.x + this.width * 0.15) {
      direction = -1;
    } else if (realPosition.x > this.position.x + this.width * 0.85) {
      direction = 1;
    } else return;
