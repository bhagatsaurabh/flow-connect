...
export class Slider extends UINode implements Serializable<SerializedSlider> {
  ...
  constructor(node: Node, public min: number, public max: number, options: SliderOptions = DefaultSliderOptions(node)) {
    super(node, Vector.Zero(), UIType.Slider, {
      draggable: true,
      style: options.style
        ? { ...DefaultSliderStyle(node, options.height), ...options.style }
        : DefaultSliderStyle(node, options.height),
