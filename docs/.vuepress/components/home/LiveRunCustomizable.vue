...
import { Timer, Random } from '@flow-connect/common/index.es';
...
onMounted(() => {
  ...
  window.customExampleFC = flowConnect.value;
  let flow = flowConnect.value.createFlow({
  ...
  let customRenderFn2 = (context, params, connector) => {
    ...
    if (params.start.x > params.end.x) {
      ...
      points.push({
        x: params.start.x + 20,
        y: params.start.y + (params.end.y - params.start.y) / 2,
      });
      points.push({
        x: params.end.x - 20,
        y: params.end.y + (params.start.y - params.end.y) / 2,
      });
  ...
  flowConnect.value.registerRenderer("connector", (connector) => {
    if (
